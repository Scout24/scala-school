object ContextBounds extends { //App {
  ...
  object MockIntComparator extends Comparator[Int] {
    override def isOrdered(a1: Int, a2: Int): Boolean = true
  }

  def orderPair[A](pair: (A, A))(implicit cp: Comparator[A]=MockIntComparator) = {
  ...
  def orderPairListExercise[A : Comparator](list: List[(A, A)]) = {
    for {pair <- list} yield orderPair(pair)
  }
